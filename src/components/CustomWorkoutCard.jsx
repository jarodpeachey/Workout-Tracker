...
const CustomWorkoutCard = ({ workout }) => {
  ...
  const handleSave = () => {
    ...
    updateWorkout(workout.id, { name: editName, exerciseIds: editExercises, is_1rm: editIs1RM });
            ...
            <div className="space-y-3">
              <div className="mb-4">
                <label className="flex items-start gap-3 cursor-pointer">
                  ...
                  />
                  <div>
                    <span className="text-md font-semibold text-black">1RM Progression Mode</span>
                    <p className="font-normal text-gray-dark text-sm">Converts exercises to 1RM format with progression sets</p>
                  </div>
                </label>
              </div>
            ...
            <>
              ...
              {items && items.length > 0 ? (
                <ul className="list-disc pl-5 space-y-1 text-black">
                  {items.map(it => (
                    <li key={it.id}>
                      {it.name} — <span className="text-gray-dark">{it.type === 'reverse' ? 'Reverse Pyramid' : it.type === 'tensetslight' ? 'UFpwrLifter Program (Light)' : 'UFpwrLifter Program'}</span>
                    </li>
                  ))}
                </ul>
              ) : (
